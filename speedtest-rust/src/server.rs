...
fn handle(stream: std::io::Result<std::net::TcpStream>) -> std::io::Result<()> {
    ...
    let protocol = match &protocol[0..protocol.len() - 1] {
        ...
        _ => {
            ...
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, format!("method not allowed: {} (must be GET or POST)", match std::str::from_utf8(&protocol) {
                Ok(str) => str,
                Err(e) => {
                    return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, format!("invalid utf8: {}", e)))
                }
            })));
        }
    };
